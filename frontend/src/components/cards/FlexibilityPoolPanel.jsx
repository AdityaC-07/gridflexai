...
export function FlexibilityPoolPanel({ feederId = 'F01' }) {
  ...
  const getDisruptionColor = (weight) => {
    if (weight <= 0.3) return { bg: '#ECFDF5', text: '#059669' };
    if (weight <= 0.6) return { bg: '#FEF3C7', text: '#D97706' };
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
        {[...Array(5)].map((_, i) => (
          <div key={i} style={{
            width: '8px',
            height: '12px',
            backgroundColor: i < bars ? '#0284C7' : '#E2E8F0',
            borderRadius: '1px'
          }} />
        ))}
  ...
  return (
    <div className="ops-panel" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="ops-panel-body" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {/* Pool Summary */}
        <div style={{
          backgroundColor: hasSufficientCoverage ? '#ECFDF5' : '#FEF2F2',
          border: `1px solid ${hasSufficientCoverage ? '#A7F3D0' : '#FCA5A5'}`,
          borderRadius: '6px',
          padding: '12px'
        }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: hasSufficientCoverage ? '#059669' : '#DC2626', marginBottom: '4px' }}>
            POOL COVERAGE
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '1.2rem', fontWeight: 800, fontFamily: 'monospace', color: hasSufficientCoverage ? '#059669' : '#DC2626' }}>
                {totalAvailable} kW
              </div>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.7rem', color: '#64748B' }}>Needed</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '1.2rem', fontWeight: 800, fontFamily: 'monospace', color: coverageRatio >= 1 ? '#059669' : '#D97706' }}>
                {(coverageRatio * 100).toFixed(0)}%
              </div>
              <div style={{ fontSize: '0.7rem', color: '#64748B' }}>Coverage</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto' }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#64748B', letterSpacing: '0.04em' }}>
            COMMUNITY RESOURCES (Ranked by RBS)
          </div>
              ...
              <div key={resource.resource_id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 600, color: '#0F172A' }}>{resource.resource_name}</div>
                      <div style={{ fontSize: '0.7rem', color: '#64748B', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        {getOwnerIcon(resource.owner_type)}
                        <span>{resource.owner_type}</span>
                        {resource.location_section && <span>· {resource.location_section}</span>}
                      </div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: 700, fontFamily: 'monospace', color: '#0F172A' }}>{resource.available_kw} kW</div>
                    <div style={{ fontSize: '0.7rem', color: '#64748B' }}>{resource.max_duration_minutes} min</div>
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginTop: '6px' }}>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: '#64748B', marginBottom: '2px' }}>Response Time</div>
                    <div style={{ fontWeight: 600, fontFamily: 'monospace' }}>{resource.response_time_minutes} min</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: '#64748B', marginBottom: '2px' }}>Disruption</div>
                    <div style={{
                      fontWeight: 600,
                      padding: '2px 6px',
                      borderRadius: '3px',
                      backgroundColor: disruptionStyle.bg,
                      color: disruptionStyle.text,
                      fontSize: '0.7rem'
                    }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.65rem', color: '#64748B', marginBottom: '2px' }}>RBS Score</div>
        ...
        {/* Pool Stats */}
        <div style={{ borderTop: '1px solid #E2E8F0', paddingTop: '10px', fontSize: '0.7rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', color: '#64748B', marginTop: '4px' }}>
            ...
            <strong>
              {resources.length > 0
                ? (resources.reduce((sum, r) => sum + r.response_time_minutes, 0) / resources.length).toFixed(1)
                : 0} min
            </strong>
          </div>
        </div>
