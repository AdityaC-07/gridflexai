...
def write_flexibility_resource(item: dict[str, Any]) -> dict[str, Any]:
    try:
        _table("flexibility-pool").put_item(Item=_to_dynamo(item))
        return item
    except Exception as exc:
        raise _friendly_error("write_flexibility_resource", exc)


def get_flexibility_resource(resource_id: str) -> dict[str, Any] | None:
    try:
        resp = _table("flexibility-pool").get_item(Key={"resource_id": resource_id})
        item = resp.get("Item")
        return _from_dynamo(item) if item else None
    except Exception as exc:
        raise _friendly_error("get_flexibility_resource", exc)


def get_feeder_flexibility_pool(feeder_id: str) -> list[dict[str, Any]]:
    try:
        resp = _table("flexibility-pool").scan(
            FilterExpression="feeder_id = :f AND enrolled = :e",
            ExpressionAttributeValues={":f": feeder_id, ":e": True}
        )
        return [_from_dynamo(i) for i in resp.get("Items", [])]
    except Exception as exc:
        raise _friendly_error("get_feeder_flexibility_pool", exc)
