...
from grid_intelligence.event_detection import check_and_create_event  # noqa: E402
...
def run_intelligence_cycle(feeder_id: str | None = None) -> dict:
    ...
    # Calculate additional grid awareness metrics
    voltage_risk = calculate_voltage_risk(solar, demand, settings.grid_import_limit_kw + 50.0)  # Assuming ~150kW feeder capacity
    transformer_loading = calculate_transformer_loading(demand, 500.0)  # 500 kVA transformer
    ...
    # Check if reliability event should be created
    try:
        event = check_and_create_event(feeder_id)
        if event:
            logger.info("Reliability event %s created for feeder %s", event["event_id"], feeder_id)
            state["reliability_event"] = event
    except Exception as exc:
