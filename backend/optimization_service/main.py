...
from optimization_service.reliability_budget import (  # noqa: E402
...
class ReliabilityEventOptimizeRequest(BaseModel):
    event_id: str
    feeder_id: str
...
@app.post("/optimization/reliability-event/optimize")
def optimize_reliability_event(body: ReliabilityEventOptimizeRequest) -> dict:
    """Run reliability budget optimization for a specific reliability event."""
    try:
        event = db.get_reliability_event(body.event_id)
        if not event:
            raise HTTPException(status_code=404, detail=f"Reliability event {body.event_id} not found")

        if event.get("status") != "PREDICTED":
            raise HTTPException(status_code=409, detail=f"Event status must be PREDICTED, got {event.get('status')}")

        predicted_gap = float(event.get("predicted_gap_kw", 0))
        duration = int(event.get("duration_minutes", 30))
        battery_soc = float(settings.battery_capacity_kwh * 0.8)  # Default to 80% if not available

        # Get actual battery state if available
        try:
            battery_state = db.get_battery_state(body.feeder_id)
            battery_soc = float(battery_state.get("soc_kwh", battery_soc))
        except Exception as exc:
            logger.warning("Failed to get battery state: %s", exc)
        ...
        result = optimize_with_reliability_budget(
            ...
            duration,
            battery_soc,
            settings.critical_load_kw
        )

        # Update event with dispatch plan
        update_event_with_dispatch_plan(body.event_id, result)
        ...
        logger.error("Reliability event optimization failed: %s", exc)
        raise HTTPException(status_code=500, detail=f"Optimization failed: {exc}")
